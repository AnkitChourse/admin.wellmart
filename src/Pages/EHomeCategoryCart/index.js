...
import SingleEHomeCategory from "./SingleEHomeCategory";
...
import { deleteSingleECategoryCart } from "redux/festures/eHomeCategoryCart";
import { getSingleECategoryCart } from "redux/festures/eHomeCategoryCart";
...
const EHomeCategoryCart = () => {
  ...
  const { Loading, EHomeCategory, singleEHomeCategory } = useSelector((state) => ({
    ...state.isEHomeCategoryCart,
  }));
  // console.log(EHomeCategory)
  useEffect(() => {
    dispatch(
  ...
  useEffect(() => {
    if (EHomeCategory && EHomeCategory.length > 0) {
      const temprows =
        EHomeCategory &&
        EHomeCategory?.at(0) &&
        EHomeCategory?.map((value, index) => ({
          no: (
          ...
          backgroundColourCode: (
            <MDBox flexDirection="column">
              ...
              <MDBadge badgeContent={value?.backgroundColourCode || "N/A"} color={value?.backgroundColourCode || "N/A"} variant="gradient" size="lg" />
              {/* <MDTypography fontSize={12.5}>Qty : {value?.couponQuantity || "N/A"}</MDTypography> */}
            </MDBox>
          ),
          taskColourCode: (
            <MDBox flexDirection="column">
                <MDBadge badgeContent={value?.taskColourCode || "N/A"} color={value?.taskColourCode || "N/A"} variant="gradient" size="lg" />
          ...
          view: (
            <IconButton
              ...
              onClick={() => {
                setIsOpenView(true);
                dispatch(getSingleECategoryCart(value));
              }}
            >
              <Visibility
  ...
  return (
    <>
        ...
        <MDBox py={3}>
          <Card>
            ...
            <MDBox py={3}>
              {Loading ? (
                <SkLoading />
              ) : EHomeCategory && EHomeCategory.length > 0 ? (
                <DataTable
                  table={{
                  ...
                  isPages={EHomeCategory && EHomeCategory.length}
                  noEndBorder
                  canSearch={false}
      ...
      <SkConfirm
        dialogTitle={"Delete"}
        dialogContent={"Are you sure you want to delete this ecommerce home category?"}
        open={isOpenDialog}
              ...
              onClick={() => {
                dispatch(
                  deleteSingleECategoryCart(
                    `${process.env.REACT_APP_APII}/eCommerce/deleteHomeCategoryCart/${isOpenDialog?.isId}/${admin}`
                  )
                ).then((data) => {
                  dispatch(
                    handleAlert({
...
export default EHomeCategoryCart;
