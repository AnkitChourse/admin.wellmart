...
import { getAllEHomeBanner } from "redux/festures/eHomeBannerSlice";
import { updateEHomeBanner } from "redux/festures/eHomeBannerSlice";
import { createEHomeBanner } from "redux/festures/eHomeBannerSlice";
...
const CreateBranner = ({ isOpen, isOpenUpdate, setIsOpenUpdate, setIsOpen }) => {
  ...
  const { Loading, AllEHomeBanner, singleEHomeBanner,createUpdateloading } = useSelector((state) => ({
    ...state.isEHomeBanner,
  }));
  console.log(singleEHomeBanner, "singleBrands");
  useEffect(() => {
    if (singleEHomeBanner && isOpenUpdate) {
      setIsBrandName(singleEHomeBanner?.title);
      setIsShow(singleEHomeBanner?.banner);
  ...
  }, [singleEHomeBanner, isOpenUpdate, isOpen]);
  ...
  const handleFormSubmit = (e) => {
    ...
    if ((isBrandImage && isBrandImage !== "") || (isShow && isShow !== "")) {
      if (isOpenUpdate) {
        ...
        dispatch(
            updateEHomeBanner({
            url: `${process.env.REACT_APP_APII}/eCommerce/updateHomeBanner/${singleEHomeBanner?._id}/${admin}`,
            data: formData,
          })
        ).then((data) => {
          dispatch(
        ...
        dispatch(
            createEHomeBanner({
            url: `${process.env.REACT_APP_APII}/eCommerce/createHomeBanner/${admin}`,
            data: formData,
          })
        ).then((data) => {
          dispatch(
      ...
      >
        ...
        >
          ...
          >
            ...
            <MDButton   disabled={createUpdateloading}color={"info"} verdant={"gradient"} type={"submit"}>
            {createUpdateloading ? (
